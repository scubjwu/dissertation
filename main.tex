...
Chapter~\ref{sec:todo} (\emph{Remaining Tasks and Time to Complete}) presents the challenges in the line of applying machine learning to help bug detection,
details the tasks to address these challenges, and provides the estimated time to complete each taks.
...
\chapter{Remaining Tasks and Time to Complete}
\label{sec:todo}
...
To generate precise error specifications based on source code,
one straightforward way is to identify \textit{error paths}
(path executions ended in error situations, for example, 
fail to open files, obtain resources, allocate memory, etc.),
and then collect the return values from error paths to construct error specifications.
%
The key observation is that we can distinguish error paths
...
%
Machine learning techniques can help learn the relationship between such features and error paths.
Therefore, we propose to use machine learning to help generate error specifications.
To this end, the following tasks have to be completed.

\begin{enumerate}

\item Design a set of features for identifying error paths.
The idea is to manually investigate the differences between error paths and non-error paths from different code bases,
so that we can find the features characterizing error paths.
The estimated time to complete this task is 1 to 2 weeks.

\item In Chapter~\ref{sec:solution}, we manually label type errors to train a model for \newCompiler.
This requires considerable efforts.
To minimize the data labeling efforts, 
I plan to design a new machine learning paradigm which can automatically label error paths for training a model.
The estimated time to complete this task is 4 to 5 weeks.

\item Evaluate the effectiveness of our solution.
I will compare the error specification accuracy between ours and those generated by the state-of-the-art approach APEx~\cite{Kang:2016:AAI:2970276.2970354}.
Furthermore, I will use the generated error specifications to detect error handling bugs in real-world projects.
The estimated time to complete this task is 2 to 3 weeks.
...
To address these issues, we use machine learning to learn the relationship between the features and error paths.
